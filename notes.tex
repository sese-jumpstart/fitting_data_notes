...
Whatever $h$ we choose should have the smallest possible difference from the data. To make the math work out better later in the process, let us calculate the total difference between the model and our data with this measure of the difference. (In math, physics and statistics this quantity mysteriously gets called $\chi^2$, that is ``chi squared,'' and in machine learning and AI it gets called the ``cost function,'' so to make the later equations cleaner let's call this quantity $C$.) The difference between the model and data is:
