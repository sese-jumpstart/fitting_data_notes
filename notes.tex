...
\fancyfoot[C]{\thepage}
\fancyfoot[R]{\small \textit{Last Edit: 8/15/2026}}
...
One note, as an illustration I used a \texttt{for} loop to perform the calculation here. In programming languages like C and rust, this is the right approach. In python, this is actually a slower way to perform this calculation, but for this case I want to show how the calculation is actually performed.
...
N = len(x)
# then initialize the sum quantity variables to zero
Sx = 0
Sy = 0
Sxx = 0
Sxy = 0
# then loop over all array elements
for i in range(N):
    # and perform the sum
    Sx = Sx + x[i]
    Sy = Sy + y[i]
